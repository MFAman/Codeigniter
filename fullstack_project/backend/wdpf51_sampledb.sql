...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 04, 2023 at 06:52 PM
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `calculate_discount` (`cust_id` CHAR(8)) RETURNS DECIMAL(10,2) COMMENT 'Calculate Customer discount' BEGIN
...
--
-- Stand-in structure for view `customerlist_with_salesrep_view`
-- (See below for the actual view)
--
CREATE TABLE `customerlist_with_salesrep_view` (
...
--
-- Stand-in structure for view `customer_france_address_view`
-- (See below for the actual view)
--
CREATE TABLE `customer_france_address_view` (
`customerNumber` int(11)
,`customerName` varchar(50)
,`addressLine1` varchar(50)
,`addressLine2` varchar(50)
...
--
-- Stand-in structure for view `customer_usa_address_view`
-- (See below for the actual view)
--
CREATE TABLE `customer_usa_address_view` (
`customerNumber` int(11)
,`customerName` varchar(50)
,`addressLine1` varchar(50)
,`addressLine2` varchar(50)
...
(1002, 'Hossen', 'Amzad', 'x5800', 'amzad@gmail.com', '1', NULL, 'President'),
...
INSERT INTO `employees2` (`employeeNumber`, `lastName`, `firstName`, `extension`, `email`, `officeCode`, `reportsTo`, `jobTitle`, `homedistrict`) VALUES
(1002, 'Hossen', 'Amzad', 'x5800', 'amzad@gmail.com', '1', NULL, 'President', ''),
(1056, 'Patterson', 'Mary', 'x4611', 'mpatterso@classicmodelcars.com', '1', 1002, 'VP Sales', ''),
(1076, 'Firrelli', 'Jeff', 'x9273', 'jfirrelli@classicmodelcars.com', '1', 1002, 'VP Marketing', ''),
(1088, 'Patterson', 'William', 'x4871', 'wpatterson@classicmodelcars.com', '6', 1056, 'Sales Manager (APAC)', ''),
...
--
-- Structure for view `customerlist_with_salesrep_view`
--
DROP TABLE IF EXISTS `customerlist_with_salesrep_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customerlist_with_salesrep_view`  AS SELECT `c`.`customerNumber` AS `customerNumber`, `c`.`customerName` AS `customerName`, `c`.`city` AS `city`, `c`.`country` AS `country`, `e`.`employeeNumber` AS `employeeNumber`, `e`.`lastName` AS `lastName`, `e`.`email` AS `email`, `e`.`jobTitle` AS `jobTitle` FROM (`customers` `c` join `employees` `e`) WHERE `e`.`employeeNumber` = `c`.`salesRepEmployeeNumber``salesRepEmployeeNumber`  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_address_view`  AS SELECT `customers`.`customerNumber` AS `customerNumber`, `customers`.`customerName` AS `customerName`, `customers`.`addressLine1` AS `addressLine1`, `customers`.`addressLine2` AS `addressLine2`, `customers`.`city` AS `city`, `customers`.`country` AS `country`, `customers`.`phone` AS `phone` FROM `customers` ORDER BY `customers`.`customerNumber` ASC  ;

-- --------------------------------------------------------

--
-- Structure for view `customer_france_address_view`
--
DROP TABLE IF EXISTS `customer_france_address_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_france_address_view`  AS SELECT `customers`.`customerNumber` AS `customerNumber`, `customers`.`customerName` AS `customerName`, `customers`.`addressLine1` AS `addressLine1`, `customers`.`addressLine2` AS `addressLine2`, `customers`.`city` AS `city`, `customers`.`country` AS `country`, `customers`.`phone` AS `phone` FROM `customers` WHERE `customers`.`country` = 'FRANCE' ORDER BY `customers`.`customerNumber` ASC  ;

-- --------------------------------------------------------

--
-- Structure for view `customer_usa_address_view`
--
DROP TABLE IF EXISTS `customer_usa_address_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_usa_address_view`  AS SELECT `customers`.`customerNumber` AS `customerNumber`, `customers`.`customerName` AS `customerName`, `customers`.`addressLine1` AS `addressLine1`, `customers`.`addressLine2` AS `addressLine2`, `customers`.`city` AS `city`, `customers`.`country` AS `country`, `customers`.`phone` AS `phone` FROM `customers` WHERE `customers`.`country` = 'USA' ORDER BY `customers`.`customerNumber` ASC  ;
